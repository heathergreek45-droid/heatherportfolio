...
const CertificationsSection = () => {
  ...
  const total = certifications.reduce((sum, cat) => sum + cat.courses.length, 0);
    ...
    <section id="certifications" className="py-24 px-6 bg-muted/30">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
          ...
          <p className="text-muted-foreground text-lg">{t("certs.subtitle", { count: total })}</p>
        </motion.div>

        <div className="grid md:grid-cols-2 gap-8">
          {certifications.map((category, ci) => (
            <motion.div key={category.category} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: ci * 0.1 }} className="bg-card border border-border rounded-xl p-6 hover:border-primary/40 transition-all hover:shadow-lg hover:shadow-primary/5">
                    ...
                    <div className="flex-1">
                      ...
                      {course.credentialId && (
                        <a
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            window.open(`https://www.coursera.org/account/accomplishments/verify/${course.credentialId}`, "_blank", "noopener,noreferrer");
                          }}
                          className="inline-flex items-center gap-1 text-xs text-primary hover:text-accent transition-colors font-medium mt-1"
                        >
                          <ExternalLink size={12} />
                          {t("certs.view_credential")}
                        </a>
                      )}
                    </div>
                  </li>
